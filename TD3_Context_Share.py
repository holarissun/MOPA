...
class Actor(nn.Module):
    def __init__(self, state_dim, action_dim, max_action, hist_len, shared_context = None):
...
class Critic(nn.Module):
    def __init__(self, state_dim, action_dim,hist_len, shared_context = None):
...
class TD3(object):
    def __init__(
        ...
        shared_context_model = Context(hidden_sizes=[30],
                                       input_dim=state_dim + action_dim + 1,
                                       output_dim = 30, # hiddens_conext
                                       only_concat_context = 3,
                                       history_length = hist_len,
        ...
        self.actor = Actor(state_dim, action_dim, max_action, hist_len,shared_context_model).to(device)
        self.actor_target = copy.deepcopy(self.actor)
        self.actor_optimizer = torch.optim.Adam(self.actor.parameters(), lr=3e-4)

        self.critic = Critic(state_dim, action_dim, hist_len,shared_context_model).to(device)
